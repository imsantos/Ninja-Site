...
(function ($) {
	$.fn.blurjs = function (options) {
		...
		return this.each(function () {
			...
			bluredElement = $(this);
			if(bluredElement.hasClass('hasBlur')){
				bluredElement.prev().animate({opacity : 1})
			} else {
				...
				tempImg.onload = function () {
					...
					$(this.el).addClass('hasBlur');
					$(this.el).addClass('base');
					
					$(this.el).before('<img src="'+canvas.toDataURL()+'" class="blur-overlay">');
					//bluredElement.prev().animate({opacity : 1})
				...
				if(cachedData != null) {
					...
					if(options.debug) {
						console.log('Source Used', formattedSource);
					}
				//	
				}
				...
				/* load image into canvas and get base64 representation */
				...
				img.onload = function(){
					...
					convertCtx.drawImage(img, 0, 0, img.width, img.height);
					convertedImageData = convertCanvas.toDataURL();
					
					tempImg.src = convertedImageData;
				};
				
				img.src = $(bluredElement).attr('src');
